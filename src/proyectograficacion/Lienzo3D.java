...
public class Lienzo3D extends JPanel {
    ...
    public String tipoVista = "Normal"; // Variable para saber qué ejes dibujar
    ...
    protected void paintComponent(Graphics g) {
        ...
        // 1. DIBUJAR LOS 3 EJES (Arriba, Derecha, Abajo-Izquierda)
        ...
        // --- CORRECCIÓN: MARCAS EXACTAS Y NÚMEROS EN LOS EJES ---
        g2d.setFont(new Font("Arial", Font.PLAIN, 10)); 
        
        int limiteUnidades = 10; 
        for (int i = 1; i <= limiteUnidades; i++) {
            // Eje X
            Point ptX = Proyecciones.proyectar(new Punto3D(i, 0, 0, 0), cx, cy, escala, tipoVista);
            g2d.setColor(Color.DARK_GRAY);
            g2d.fillOval(ptX.x - 2, ptX.y - 2, 4, 4); // Puntito oscuro marcando la coordenada exacta
            g2d.setColor(Color.GRAY);
            g2d.drawString(String.valueOf(i), ptX.x + 5, ptX.y + 12);

            // Eje Y
            Point ptY = Proyecciones.proyectar(new Punto3D(0, i, 0, 0), cx, cy, escala, tipoVista);
            g2d.setColor(Color.DARK_GRAY);
            g2d.fillOval(ptY.x - 2, ptY.y - 2, 4, 4); // Puntito oscuro marcando la coordenada exacta
            g2d.setColor(Color.GRAY);
            g2d.drawString(String.valueOf(i), ptY.x + 5, ptY.y + 12);

            // Eje Z
            Point ptZ = Proyecciones.proyectar(new Punto3D(0, 0, i, 0), cx, cy, escala, tipoVista);
            g2d.setColor(Color.DARK_GRAY);
            g2d.fillOval(ptZ.x - 2, ptZ.y - 2, 4, 4); // Puntito oscuro marcando la coordenada exacta
            g2d.setColor(Color.GRAY);
            g2d.drawString(String.valueOf(i), ptZ.x + 5, ptZ.y + 12);
        }
        // -------------------------------------------------------------

        // 2. ETIQUETAS DE LOS EJES (X, Y, Z) SEGÚN LA VISTA
        g2d.setColor(Color.BLACK);
        g2d.setFont(new Font("Arial", Font.BOLD, 16));
        
        String labelArriba = "Z";
        String labelDerecha = "Y";
        String labelDiagonal = "X";

        if ("RotacionY".equals(tipoVista)) {
            labelArriba = "X"; labelDerecha = "Z"; labelDiagonal = "Y";
        } else if ("RotacionZ".equals(tipoVista)) {
            labelArriba = "Y"; labelDerecha = "X"; labelDiagonal = "Z";
        }
        ...
        // 3. Dibujar Figuras (INTACTO)
        dibujarLineasPunteadas(g2d, figuraOriginal, figuraTransformada, cx, cy, escala);

        if (figuraOriginal != null) {
            dibujarPoliedro(g2d, figuraOriginal, cx, cy, escala, new Color(0, 0, 255, 180), "P");
        }

        if (figuraTransformada != null) {
            dibujarPoliedro(g2d, figuraTransformada, cx, cy, escala, new Color(255, 0, 0, 180), "P'");
        }
    }
